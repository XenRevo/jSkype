...
public class Group {
    ...
    public boolean kick(SkypeAPI api, String username) {
        return new UserManagementPacket(api, api.getUser()).kickUser(chatId, username.toLowerCase());
    }
    /** Remove a user from the group - must be MASTER */
    ...
    public boolean add(SkypeAPI api, String username) {
        return new UserManagementPacket(api, api.getUser()).addUser(chatId, username.toLowerCase());
    }
    /** Add a user to the group */
